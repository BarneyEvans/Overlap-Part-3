...
def split_info_loader_helper(func):
    ...
    def wrapper(*args, **kwargs):
        ...
        if not osp.isfile(split_file_path):
            ...
            print(f"Split file: {split_file_path}")  # Add this line
            print(f"Contents of split file: {split_list}")  # Add this line
        return split_list
    return wrapper
...
if __name__ == '__main__':
        ...
        for cam_name, img_buf in img_buf_dict.items():
            cv2.imshow(f'Box Projection {cam_name} {frame_id}', cv2.cvtColor(img_buf, cv2.COLOR_BGR2RGB))
            img_file_path = f'C:\\Users\\evans\\OneDrive\\Desktop\\University\\Year 3\\Part 3 Project\\ONCE Dataset\\Results\\Actual Results\\3D Boxes\\{cam_name}_{frame_id}.png'
            cv2.imwrite(img_file_path, cv2.cvtColor(img_buf, cv2.COLOR_BGR2RGB))
            cv2.waitKey(0)  # Wait for a key press to continue
            cv2.destroyAllWindows()  # Close the window

        img_buf_dict = dataset.project_lidar_to_image(seq_id, frame_id)
        for cam_name, img_buf in img_buf_dict.items():
            cv2.imshow(f'Lidar Projection {cam_name} {frame_id}', cv2.cvtColor(img_buf, cv2.COLOR_BGR2RGB))
            img_file_path = f'C:\\Users\\evans\\OneDrive\\Desktop\\University\\Year 3\\Part 3 Project\\ONCE Dataset\\Results\\Actual Results\\Lidar-Image\\{cam_name}_{frame_id}.png'
            cv2.imwrite(img_file_path, cv2.cvtColor(img_buf, cv2.COLOR_BGR2RGB))
            cv2.waitKey(0)  # Wait for a key press to continue
            cv2.destroyAllWindows()  # Close the window

        for cam_id in ['cam01', 'cam03', 'cam05', 'cam06', 'cam07', 'cam08', 'cam09']:
            image = dataset.load_image(seq_id, frame_id, cam_id)
            img_file_path = f'C:\\Users\\evans\\OneDrive\\Desktop\\University\\Year 3\\Part 3 Project\\ONCE Dataset\\Results\\Actual Results\\Images\\cam{cam_id}_{frame_id}.jpg'
